...
def view_inventory():
    ...
    try:
        ...
        for asset in assets:
            ...
            print(
                f"  - Modelo: {asset['device_model'] if 'device_model' in asset.keys() else 'Desconhecido'}")
            print(f"  - SO: {asset['os']} ({asset['architecture']})")
            print(
                f"  - IP: {asset['ip_address']:<15} | MAC: {asset['mac_address']}")

            # --- LINHA MODIFICADA ---
            # Adicionado o 'cpu_model' que já estava sendo salvo no banco de dados.
            print(
                f"  - CPU: {asset['cpu_model']} ({asset['cpu_cores_physical']} Cores Físicos / {asset['cpu_cores_logical']} Threads)")
            ...
            if disks_data:
                ...
                for disk in disks_data:
                    print(
                        f"    - {disk['mountpoint']} {disk['total_gb']:.2f} GB (Usado: {disk['used_gb']:.2f} GB - {disk['percent_used']}%)")
